...
function solution(want, number, discount) {
    ...
    let wantedProductCount = {};
    let product = {};
    let result = 0;

    for(let i = 0; i < want.length; i ++) {
        wantedProductCount[want[i]] = number[i];
        product[want[i]] = 0;
    }
    product = productInit(product);

    if(discount.length == 10) {
        if(Object.entries(product).toString() == Object.entries(wantedProductCount).toString()) {
            return 1;
        }
        return 0;
    }
    ...
    for(let i = 10; i < discount.length; i++) {
        let [outSale, inSale] = [discount[i-10], discount[i]];

        if(Object.keys(product).includes(outSale)) {
            product[outSale] --;
        }

        if(Object.keys(product).includes(inSale)) {
            product[inSale] ++;
        }
